...
class NormAttentionPSN(nn.Module):
    ...
    def forward(self, x):
        ...
        feats6 = torch.Tensor()
        feats10 = torch.Tensor()
        feats13 = torch.Tensor()
        atts = torch.Tensor()
        for i in range(len(img_split)):
            ...
            if i == 0:
                ...
                if self.fuse_type == 'mean':
                    feats6 = torch.stack([feats6, feat6], 1).sum(1)
                    feats10 = torch.stack([feats10, feat10], 1).sum(1)
                    feats13 = torch.stack([feats13, feat13], 1).sum(1)
                    atts = torch.stack([atts, att], 1).sum(1)
                elif self.fuse_type == 'max':
                    feats6, _ = torch.stack([feats6, feat6], 1).max(1)
                    feats10, _ = torch.stack([feats10, feat10], 1).max(1)
                    feats13, _ = torch.stack([feats13, feat13], 1).max(1)
                    atts, _ = torch.stack([atts, att], 1).max(1)
        if self.fuse_type == 'mean':
            feats6 = feats6 / len(img_split)
            feats10 = feats10 / len(img_split)
            feats13 = feats13 / len(img_split)
            atts = atts / len(img_split)
        feat_fused6 = feats6
        feat_fused10 = feats10
        feat_fused13 = feats13
        att_fused = atts
