...
class Logger(object):
    ...
    def saveNormalResultsmat(self, results, split, epoch, iters):
        save_dir = os.path.join(self.args.log_dir, split)
        results = np.array(results.cpu())
        save_name = '%d_%d.mat' % (epoch, iters)
        io.savemat(os.path.join(save_dir, save_name),{'normalEst':results})
    ...
    def saveAttentionResults(self, results, split, epoch, iters):
        save_dir = os.path.join(self.args.log_dir, split)
        save_name = 'att%d_%d.png' % (epoch, iters)
        vutils.save_image(results, os.path.join(save_dir, save_name), padding=0)
