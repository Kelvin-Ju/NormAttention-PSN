...
def test(args, split, loader, model, log, epoch, recorder):
    ...
    with torch.no_grad():
        for i, sample in enumerate(loader):
            ...
            out_var, attention  = model(input); timer.updateTime('Forward')
            ...
            if iters % save_intv == 0:
                ...
                log.saveNormalResultsmat(masked_pred, split, epoch, iters)
                log.saveErrorMap(error_map, split, epoch, iters)

            if iters % save_intv == 0:
                pred = (attention.data + 1) / 2
                masked_pred = pred * data['m'].data.expand_as(attention.data)
                log.saveAttentionResults(masked_pred, split, epoch, iters)
